...
import { Notice } from '../obsidian_api/vscode_impl';
...
export class LocalModel implements Model {
    ...
    async imgfileToLatex(filepath: PathLike): Promise<string> {
        if (!(typeof (filepath) === "string")) { return new Promise((resolve, reject) => { resolve(""); });}
    ...
    checkPythonInstallation() {
        return new Promise<void>(
            (resolve, reject) => {
                let args = ["--version"];
                let is_not_python = this.plugin_settings.pythonPath.includes("latex-pix-server");
                if (!is_not_python) {
                    args = ["-m", "latex_ocr_server", ...args];
                }
    ...
    spawnLatexOcrServer(port: string): Promise<ChildProcess> {
        return new Promise<ChildProcess>((resolve, reject) => {
            ...
            // const terminal = vscode.window.createTerminal({ name: "latex-pix-server", hideFromUser: true });
            // terminal.sendText(`${this.plugin_settings.pythonPath} ${args.join(" ")}`, true);
